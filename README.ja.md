...
言語固有のテキスト処理については、xer は原則として英語と日本語に対象を絞ります。あらゆる文字体系を広く浅く扱う Unicode ライブラリを目指すのではなく、必要な場面では日本語サポートの品質、一貫性、予測しやすさを優先します。
...
- 文字・文字列処理
- Unicode コードポイント走査、書記素クラスタ走査、NFC 正規化
- 入出力とファイルシステム周辺機能
...
- 挙動が明示的で予測しやすいこと
- ロケール依存を極力避けること
- 英語と日本語を中心に、実用的な Unicode 対応を行うこと
- 日本語サポートを手厚くすること
...
- 言語固有の処理は、原則として英語と日本語を対象にする
- 多言語対応をいたずらに広げるより、日本語処理の品質と予測しやすさを優先する
...
xer には、通常のツールやコード例で使いやすい小さな日本語処理部品も含めています。

- 漢数字変換
- ふりがな整形
- MeCab による形態素解析と分かち書き補助
- MeCab の読みを使った仮名・ローマ字分かち書き
- 点字前置符定数、低水準の文字変換、仮名列変換、英数字記号の自動モード切替、情報処理点字、MeCab 連携の点字変換

点字関連機能は実用的な部品群です。完全な点訳エンジンを名乗るものではなく、MeCab 連携の変換は外部 MeCab 辞書が返す読みに依存します。

xer は、この日本語重視の方針を意図的に採ります。英語・日本語以外の言語や文字体系については、必要なユーザーが公開ソースコードを拡張するか、専用の Unicode ライブラリと組み合わせて扱う前提です。
