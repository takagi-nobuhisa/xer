...
private:
    ...
    [[nodiscard]] auto parse_escape_sequence() -> result<std::u8string>
    {
        if (pos >= text.size()) {
            return std::unexpected(make_error(error_t::invalid_argument));
        }
    ...
    [[nodiscard]] auto parse_unicode_escape() -> result<std::u8string>
    {
    ...
    [[nodiscard]] auto parse_hex16() -> result<char32_t>
    {
        if (pos + 4 > text.size()) {
            return std::unexpected(make_error(error_t::invalid_argument));
        }
    ...
    [[nodiscard]] auto parse_utf8_code_point(char8_t first_byte) -> result<char32_t>
    {
 ...
 */
[[nodiscard]] inline auto json_decode(std::u8string_view json_text) -> result<json_value>
{
