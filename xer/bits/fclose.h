 ...
 */
[[nodiscard]] inline std::expected<void, error<void>> fclose(binary_stream& stream) noexcept {
    if (stream.close() < 0) {
 ...
 * @return Unexpected error on failure.
 */
[[nodiscard]] inline std::expected<void, error<void>> fclose(text_stream& stream) noexcept {
    if (stream.close() < 0) {
        return std::unexpected(make_error(error_t::runtime_error));
    }
