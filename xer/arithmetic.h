/**
 * @file xer/arithmetic.h
 * @brief Public arithmetic concepts and arithmetic/comparison functions.
 */

#pragma once

#ifndef XER_ARITHMETIC_H_INCLUDED_
#define XER_ARITHMETIC_H_INCLUDED_

#include <xer/bits/arithmetic_concepts.h>
#include <xer/bits/basic_arithmetic.h>
#include <xer/bits/div.h>
#include <xer/bits/floating_arithmetic.h>
#include <xer/bits/power.h>
#include <xer/bits/compare.h>
#include <xer/bits/in_range.h>
#include <xer/bits/is_close.h>
#include <xer/bits/minmax.h>

#endif /* XER_ARITHMETIC_H_INCLUDED_ */
