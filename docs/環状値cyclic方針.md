...
* 返り値は `0` 以上 `1` 未満とする
* 返り値は `cyclic<T>` ではなく、**1 周を 1 とする実数値**である
...
`diff` の返り値も、`cyclic<T>` ではなく、**1 周を 1 とする符号付き実数値**である。
...
* `cw`, `ccw`, `diff` の返り値のような **1 周を 1 とする実数値**も、同じ変換系で扱える方が自然である
...
template <std::floating_point T>
auto to_degree(T value) noexcept -> T;
...
`to_degree(cyclic<T>)` は、`cyclic<T>` の内部正規化値を度数法へ変換する。
...
`to_degree(T)` は、**1 周を 1 とする実数値**を、そのまま度数法へ換算する関数とする。

これは、少なくとも次のような値を自然に degree へ変換するために用いる。

* `cw` の返り値
* `ccw` の返り値
* `diff` の返り値
* それ以外の turn-based scalar 値

この関数は **正規化を行わない**。
したがって、負値や `1` 以上の値もそのまま degree へ換算してよい。
...
`from_degree(T)` は degree 値を `cyclic<T>` へ変換する。

このとき、`cyclic<T>` の構築規則に従って `[0, 1)` へ正規化される。

したがって、たとえば次のようになる。

* `from_degree(90)` → `0.25`
* `from_degree(450)` → `0.25`
* `from_degree(-90)` → `0.75`
...
これらを degree へ変換する場合は、`to_degree(T)` を用いてよい。

* `to_degree(a.ccw(b))` は `72`
* `to_degree(a.cw(b))` は `288`
...
* `to_degree(a.diff(b))` は `72`

となる。

また、`0.1` から `0.9` への最短差は `-0.2` であり、
...
template <std::floating_point T>
constexpr auto to_degree(T value) noexcept -> T;
...
* API 名は過度に冗長でないこと
* `cyclic<T>` 自体と、1 周を 1 とする実数値との両方を自然に単位変換できること
